...
def main():
    ...
    print("="*80)
    print(f"{'ECC Type':<12} {'Capacity':<8} {'CE':<8} {'UE':<8} {'SDC':<8} {'Total':<10}")
    print("-"*80)
    ...
    for result in results:
        print(f"{result['ecc_type']:<12} {result['capacity']:<8} "
              f"{result['ce']:<8} {result['ue']:<8} {result['sdc']:<8} {result['total']:<10}")
    ...
    try:
        with open(csv_filename, 'w', encoding='utf-8') as f:
            # 헤더 작성
            f.write("ECC Type,Capacity,CE,UE,SDC,Total\n")
            
            # 데이터 작성
            for result in results:
                f.write(f"{result['ecc_type']},{result['capacity']},{result['ce']},{result['ue']},{result['sdc']},{result['total']}\n")
